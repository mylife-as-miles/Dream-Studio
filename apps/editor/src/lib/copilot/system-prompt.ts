...
export function buildSystemPrompt(editor: EditorCore): string {
  ...
  const lines = [
    ...
    "- For structured articulated 3D assets, use `create_articulated_asset` instead of loosely placing unrelated props.",
    "- Think in Articraft terms: one named asset root, semantic parts, reusable materials, and explicit joints/articulations.",
    ...
    "- The viewport representation is a structured editor asset made from scene nodes and metadata; it is not a standalone Morphus HTML game.",
    "",
...
export function buildMorphusSystemPrompt(): string {
  return [
    ...
    "- For single-file games, return `index.html` as one complete HTML file.",
    "- For multi-file games, return one fenced code block per file and label each block with a filename, for example ```html filename=\"index.html\" or ```javascript filename=\"index.js\".",
    ...
    "- After the file code blocks, call `generate_game_html` with a short title.",
    "",
    "## ElevenLabs Audio",
