...
export function createAnimationGameSyncPlugin() {
  return {
    name: "animation-studio-game-sync",
    configureServer(server: any) {
      registerAnimationGameSyncApi(server)
      registerAnimationStudioPresence(server)
    },
...
function registerAnimationGameSyncApi(server: any) {
...
function registerAnimationStudioPresence(server: any) {
  if (!server.httpServer) {
    return
  }

  const registrationId = `animation-studio:${server.config.root}`
