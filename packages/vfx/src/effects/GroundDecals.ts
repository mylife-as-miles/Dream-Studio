 ...
 */
/** One pooled ground decal. */
export type DecalRecord = {
  mesh: Mesh;
  ...
  life: number;
  radius: number;
  /** Metres the footprint grows over its life. */
  growth: number;
};

export type DecalSpawnOptions = {
  radius?: number;
  life?: number;
  colorA?: Color | null;
  colorB?: Color | null;
...
export class DecalSystem {
  ...
  private poolFor(type: number): ObjectPool<DecalRecord> {
    let pool = this.pools.get(type);
    if (!pool) {
      pool = new ObjectPool(() => this.createDecal(type), (decal: DecalRecord) => {
  ...
  private createDecal(type: number): DecalRecord {
    const additive =
   ...
   */
  spawn(type: number, position: Vector3, options: DecalSpawnOptions = {}): DecalRecord {
    const {
      radius = 2,
      life = 2,
      colorA = null,
    ...
    const decal = this.poolFor(type).acquire();
  ...
  update(dt: number): void {
    for (let i = this.active.length - 1; i >= 0; i--) {
      ...
      if (t >= 1) {
        ...
        this.poolFor(decal.type).release(decal);
      }
    }
  }

  clear() {
    for (const decal of this.active) this.poolFor(decal.type).release(decal);
    this.active.length = 0;
  }

  dispose() {
    this.clear();
    for (const pool of this.pools.values()) pool.dispose((decal: DecalRecord) => decal.material.dispose());
    this.pools.clear();
