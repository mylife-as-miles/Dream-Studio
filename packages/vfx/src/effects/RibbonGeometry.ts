 ...
 */
/** Everything `RibbonGeometry.build` accepts. */
export type RibbonModeId = (typeof RibbonMode)[keyof typeof RibbonMode];

export type RibbonBuildOptions = {
  /** Points to use. Defaults to `points.length`. */
  count?: number;
  width?: number;
  mode?: RibbonModeId;
  /** Required by the billboard mode, which faces the strip at the camera. */
  cameraPosition?: Vector3 | null;
  /** Required by the flat mode, which pins the strip to a plane. */
  normal?: Vector3 | null;
  /** Width multiplier along the ribbon, sampled 0..1. */
  /** Width multiplier along the ribbon: `t` is 0..1, `i` the sample index. */
  widthProfile?: ((t: number, i: number) => number) | null;
  twist?: number;
  twistPhase?: number;
};
...
export class RibbonGeometry {
   ...
   */
  build(points: Vector3[], options: RibbonBuildOptions = {}) {
    ...
    // `attributes` is typed as the union three allows; every one of these was
    // created here as a plain BufferAttribute.
    const attributes = this.geometry.attributes as Record<string, BufferAttribute>;
    const { position, uv, aDist: dist, aNormal: normals, aCenter, aTangent } = attributes;

    const uploads: Array<[BufferAttribute, number]> = [
      [position, 3],
      [normals, 3],
      [uv, 2],
      [dist, 1]
