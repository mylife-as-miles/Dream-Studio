...
export class PyreAbility extends Ability {
  material!: ReturnType<typeof createPyreMaterial>;
  _shapeKey!: string;
  meshes!: InstancedMesh[];
  seedAttributes!: InstancedBufferAttribute[];
  birthAttributes!: InstancedBufferAttribute[];
  growAttributes!: InstancedBufferAttribute[];
  charAttributes!: InstancedBufferAttribute[];
  fieldGeometry!: PlaneGeometry;
  fieldMaterial!: ReturnType<typeof createEmberFieldMaterial>;
  field!: Mesh;
  veilGeometry!: CylinderGeometry;
  veilMaterial!: ReturnType<typeof createFlameVeilMaterial>;
  veil!: Mesh;
  hazeGeometry!: CylinderGeometry;
  hazeMaterial!: ReturnType<typeof createHeatHazeMaterial>;
  haze!: Mesh;
  records!: PyreBladeRecord[];
  _activeCount!: number;
  _drawn!: number;
  _seed!: number;
  _openTime!: number;
  _entryAngle!: number;
  _scorchDistance!: number;
  _state!: { centre: Vector3 };
  _fieldState!: { radius: number; quadSize: number; burn: number; fade: number; seed: number };
  _veilState!: { fade: number; seed: number };
  _hazeState!: { fade: number; seed: number };
  smoke!: ParticleSystem;
  cinders!: ParticleSystem;
  embers!: ParticleSystem;
  updraft!: ParticleSystem;
  smokeEmitter!: RateEmitter;
  emberEmitter!: RateEmitter;
  updraftEmitter!: RateEmitter;
  scorchEmitter!: RateEmitter;
  ringEmitter!: RateEmitter;
