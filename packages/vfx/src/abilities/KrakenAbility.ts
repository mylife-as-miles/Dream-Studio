...
export class KrakenAbility extends Ability {
  material!: ReturnType<typeof createKrakenMaterial>;
  _shapeKey!: string;
  meshes!: InstancedMesh[];
  shapeAttributes!: InstancedBufferAttribute[];
  waveAttributes!: InstancedBufferAttribute[];
  lifeAttributes!: InstancedBufferAttribute[];
  fieldGeometry!: PlaneGeometry;
  fieldMaterial!: ReturnType<typeof createAbyssFieldMaterial>;
  field!: Mesh;
  veilGeometry!: CylinderGeometry;
  veilMaterial!: ReturnType<typeof createBrineVeilMaterial>;
  veil!: Mesh;
  records!: KrakenArmRecord[];
  _activeCount!: number;
  _drawn!: number;
  _seed!: number;
  _openTime!: number;
  _entryAngle!: number;
  _slickDistance!: number;
  _finaleFired!: boolean;
  _state!: { centre: Vector3 };
  _fieldState!: { radius: number; quadSize: number; open: number; fade: number; seed: number };
  _veilState!: { fade: number; seed: number };
  _pose!: { lean: number; curl: number; wave: number; twist: number; flash: number; squash: number };
  _strike!: { time: number; finale: boolean; wind: number };
  ink!: ParticleSystem;
  spray!: ParticleSystem;
  debris!: ParticleSystem;
  motes!: ParticleSystem;
  inkEmitter!: RateEmitter;
  sprayEmitter!: RateEmitter;
  moteEmitter!: RateEmitter;
  slickEmitter!: RateEmitter;
  rippleEmitter!: RateEmitter;
