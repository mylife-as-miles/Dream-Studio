...
async function createGeometryObject(
  ...
  for (const primitive of geometry.primitives) {
    ...
    applyRuntimeSurfaceAttributes(geometry, primitive);
  ...
  for (const decal of geometry.decals ?? []) {
    group.add(await createRuntimeProjectedDecalMesh(decal, (path) =>
      loadTexture(path, createDecalTextureMaterialSpec(decal), "surfaceDecalTexture", textureCache, options)
    ));
  }
...
async function createThreeMaterial(
  ...
  const blendLayerTextures = await Promise.all(
    (materialSpec.blendLayers ?? []).slice(0, 4).map((layer) =>
      layer.colorTexture
        ? loadTexture(layer.colorTexture, materialSpec, "surfaceBlendColorTexture", textureCache, options)
  ...
  if (materialSpec.baseColorTexture) {
    ...
    const texture = await loadTexture(WHITE_TEXTURE_DATA_URI, materialSpec, "baseColorTexture", textureCache, options);
    texture.colorSpace = SRGBColorSpace;
    material.map = texture;
  }
