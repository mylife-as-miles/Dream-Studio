...
const CANDIDATES: LaunchRecipe[] = [
  ...
  { headless: false, channel: 'chrome', args: ['--enable-unsafe-webgpu', '--ignore-gpu-blocklist'] },
  {
    headless: false,
    channel: 'chrome',
    args: ['--enable-unsafe-webgpu', '--ignore-gpu-blocklist', '--enable-features=Vulkan,WebGPU', '--use-angle=vulkan'],
  },
  { headless: true, channel: 'chrome', args: ['--enable-unsafe-webgpu', '--ignore-gpu-blocklist'] },
...
const DEFAULT_LAAS_BASE = process.env['LAAS_BASE_URL'] ?? 'http://127.0.0.1:5001/procedural-world-verification.html';
const PROBE_BASE = new URL(DEFAULT_LAAS_BASE).origin;
...
async function probeRecipe(recipe: LaunchRecipe): Promise<Browser | null> {
  ...
  try {
    ...
    const probe = await page.evaluate(async () => {
      ...
      if (!gpu) return { adapter: false, navigatorGpu: false };
      const adapter = await gpu.requestAdapter();
      return { adapter: adapter !== null, navigatorGpu: true };
    });
    await page.close();
    console.log(`[launch] probe headless=${recipe.headless} channel=${recipe.channel ?? 'default'} args=[${recipe.args.join(' ')}] browser=${browser.version()} result=${JSON.stringify(probe)}`);
    if (probe.adapter) return browser;
    await browser.close();
    return null;
  } catch (error) {
    console.warn(`[launch] probe failed headless=${recipe.headless} channel=${recipe.channel ?? 'default'} args=[${recipe.args.join(' ')}]: ${error instanceof Error ? error.message : String(error)}`);
...
export async function launchWebGPU(): Promise<{ browser: Browser; recipe: LaunchRecipe }> {
  ...
  throw new Error(
    `No Chromium launch recipe produced a WebGPU adapter (requires the dev server at ${PROBE_BASE} for the secure-context probe). ` +
...
export function laasUrl(opts: LaasPageOptions, base = DEFAULT_LAAS_BASE): string {
